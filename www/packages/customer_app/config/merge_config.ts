...
export type MergeRecursively<T, U> =
  | Extract<T | U, Primitive>
  | MergeArrays<Extract<T, readonly any[]>, Extract<U, readonly any[]>>
  | MergeObjects<
      Exclude<T, Primitive | readonly any[]>,
...
type MergeTupleFold<T extends any[], TRest> = T extends [infer A]
  ? MergeRecursively<A, TRest>
  : T extends [...infer TList, infer A]
    ? MergeTupleFold<TList, A>
    : never;

export type MergeTuple<T extends any[]> = T extends [...infer List, infer A]
  ? MergeTupleFold<List, A>
  : { ERROR: "TUPLE MUST HAVE AT LEAST ONE ELEMENT TO ITERATE OVER" };
